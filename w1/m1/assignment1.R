...
# read in data
week1_ivan<-read.csv("week1_ivan.csv")
week1_alavaro<-read.csv("week1_alavaro.csv")
week1_jin<-read.csv("week1_jin.csv")
week1_Yingxue<-read.csv("week1_Yingxue.csv")
...
# left join combines two datasets at a time
week1 <- left_join(
  week1_ivan,
  week1_jin,
  by = c("review_id", "text")
...
select(week1_ivan, -text)


# as are these:

names(left_join(week1_ivan,
                week1_jin,
                by = c("review_id", "text")))

left_join(week1_ivan,
          week1_jin,
          by = c("review_id", "text")) %>%
  names()
...
week1_humans %>%
  mutate_at(c("stars_IVAN","stars_jin","stars_alavaro","stars_Yingxue"),~.+1) %>%
  head()
